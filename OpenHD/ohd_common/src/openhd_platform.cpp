...
// Constants
static constexpr auto NVIDIA_XAVIER_BOARDID_PATH = "/proc/device-tree/nvidia,dtsfilename";
...
static int internal_discover_platform() {
    ...
    if (OHDFilesystemUtil::exists(ALLWINNER_BOARDID_PATH)) {
      ...
      const auto filename_proc_cpuinfo = "/proc/cpuinfo";
        ...
        const std::string compatible_content = OHDFilesystemUtil::read_file(DEVICE_TREE_COMPATIBLE_PATH);
        const std::string device_tree_model = OHDFilesystemUtil::read_file("/proc/device-tree/model");
        ...
        if (regex_search(compatible_content, sm, r)) {
            ...
            if (chip == "rk3588") {
                if (OHDUtil::contains_after_uppercase(device_tree_model, "Radxa ROCK 5A")) {
                    return X_PLATFORM_TYPE_ROCKCHIP_RK3588_RADXA_ROCK5_A;
                } else {
                    return X_PLATFORM_TYPE_ROCKCHIP_RK3588_RADXA_ROCK5_B;
                }
            } else if (chip == "rk3566") {
                if (OHDUtil::contains_after_uppercase(device_tree_model, "Radxa CM3 RPI CM4 IO")) {
